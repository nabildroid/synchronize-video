...
const Button: React.FC<Props> = ({ text, icon = null, className = "", iconSize, textSize = "md", fullwith = false, click = null, type = "button" }) => {
    ...
    return (
        <button
            ...
            type={type}
        >
            {icon && <Svg type={icon} color={TWColors.WHITE} size={iconSize} />}
            <span>{text}</span>
        </button>
    )
}
